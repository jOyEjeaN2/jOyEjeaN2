## Hi! I'm YeJin ✌️ 
***AI & ML Engineer***

![Gitanimals Farm](https://render.gitanimals.org/farms/jOyEjeaN2)


&nbsp; 

***💻 Feautred Projects***
| Project | Description | Github |
| :---: | :---: | :---: |
| **CodoC** | AI 기반 코딩 테스트 문해력 향상 및 퀴즈 학습 플랫폼 | [CodoC Github](https://github.com/orgs/100-hours-a-week/teams/12-1/repositories) |
| **실록샐록** | 조선왕조실록 데이터 기반 역사학습 챗봇 (KCI 논문 게재) | [실록샐록 Github](https://github.com/HSORI-SLSL) |

&nbsp; 

***⚡️ Highlights***
- **KCI 논문 등재** -  BERT-LLaMA2 하이브리드 모델 기반 역사 학습 챗봇 연구
- **Kakao Tech Bootcamp** -  3기 수료 및 부하 테스트 우수상 수상

&nbsp; 


***Using Skills***
| Category | Skills |
| :--- | :--- |
| **Languages** | ![Python](https://img.shields.io/badge/Python-3776AB?style=social&logo=python) |
| **AI / LLM** | ![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=social&logo=pytorch) ![FastAPI](https://img.shields.io/badge/FastAPI-005571?style=social&logo=fastapi) ![LangGraph](https://img.shields.io/badge/LangGraph-1C3C3C?style=social&logo=langgraph) ![RAG](https://img.shields.io/badge/RAG-FF6F61?style=social&logo=quicklook) |
| **Database** | ![Qdrant](https://img.shields.io/badge/Qdrant-FF4B26?style=social&logo=qdrant) |
| **Tools** | ![Git](https://img.shields.io/badge/Git-F05032?style=social&logo=git) |

***Used Skills*** </br>
![React](https://img.shields.io/badge/React-61DAFB?style=social&logo=react) ![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=social&logo=html5) ![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=social&logo=css3)


<!--
![Gitanimals](https://render.gitanimals.org/lines/jOyEjeaN2)
-->
